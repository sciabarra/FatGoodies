...
This will listen on port 1+<port> and proxy through to <host>:<port>
...
counter = int(now.strftime("%s"))
try: os.makedirs(dir)
except: pass

file = "%s/%d.txt" % (dir,counter)
outfile = open(file, "w")
...
class proxy_sender (asynchat.async_chat):
	...
	def handle_close (self):
		print 'Sender closing (inbuf len %d )' % ( len( self.buffer ))
		
		if len( self.buffer ):
			self.found_terminator()
	
		global outfile, file, dir, counter
		print "wrote %s" % file	
		#close(outfile)
		counter = counter + 1
		file = "%s/%d.txt" % (dir, counter)
...
class proxy_receiver (asynchat.async_chat):
	...
	def handle_close (self):
		print 'Receiver closing (inbuf len %d )' % (
			len( self.buffer ))
		...
		# next...
		now = datetime.datetime.today()
		outfile = open(dir+now.strftime("/%s.txt"), "w")
